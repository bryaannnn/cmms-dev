...
import { Plus, Upload, Filter, ChevronDown, Trash2, Edit, Eye, X, AlertTriangle, Clock, Search } from "lucide-react";
import PageHeader from "../../PageHeader";
import { WorkShift, useAuth } from "../../../routes/AuthContext";
...
const WorkShiftPage: React.FC = () => {
  ...
  const loadWorkShift = useCallback(async () => {
    try {
      ...
      const sortedWorkShift = [...dataWorkShift].sort((a, b) => a.id - b.id);
      setWorkShift(sortedWorkShift);
    } catch (err) {
      setError("Failed to load work shifts");
  ...
  const filteredWorkShifts = useMemo(() => {
    ...
    return workShift.filter((shift) => {
      const matchesWorkShift = shift.name?.toLowerCase().includes(actualQuery);
      const matchesDescription = shift.description?.toLowerCase().includes(actualQuery);

      if (category === "work shift") return matchesWorkShift;
      if (category === "description") return matchesDescription;

      return matchesWorkShift || matchesDescription;
    });
  }, [workShift, searchQuery, searchCategories]);

  const handleDelete = async (id: number) => {
    try {
      await deleteWorkShift(id);
      setWorkShift(workShift.filter((shift) => shift.id !== id));
  ...
  return (
    <div className="flex h-screen font-sans antialiased bg-blue-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <PageHeader mainTitle="Work Shift" mainTitleHighlight="Page" description="Manage work shifts and their configurations within the system." icon={<Clock />} isMobile={isMobile} toggleSidebar={toggleSidebar} />
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar bg-gray-50">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between space-y-5 md:space-y-0">
            <div>
              ...
              <p className="text-gray-600 mt-2 text-sm max-w-xl">Organize and manage work shifts by specific company requirements.</p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
            <StatCard title="Total Work Shifts" value={workShift.length.toString()} change={`+${Math.floor((workShift.length / 10) * 100)}%`} icon={<Clock className="w-6 h-6" />} />
            <StatCard
              title="Active Shifts"
              value={workShift.filter((shift) => shift.description && shift.description.length > 0).length.toString()}
              change={`+${Math.floor((workShift.filter((shift) => shift.description && shift.description.length > 0).length / workShift.length) * 100)}%`}
            ...
            <div className="flex flex-col md:flex-row md:items-center md:space-x-4 space-y-4 md:space-y-0 relative">
              <div className="flex-1 relative">
                ...
                <input
                  ref={searchInputRef}
                  type="text"
                  placeholder="Search by work shift or description..."
          ...
          {loading ? (
            <div className="bg-white rounded-2xl shadow-md p-8 text-center border border-blue-100">
              ...
              <p className="text-gray-600 text-base font-medium">Loading work shift data...</p>
            </div>
          ) : filteredWorkShifts.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-md p-8 text-center border border-blue-100">
              <p className="text-gray-700 text-base font-medium">{searchQuery ? "No work shifts found matching your search." : "No work shifts available."}</p>
              {searchQuery && (
                    ...
                    {filteredWorkShifts.map((shift) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3">
                          <div className="text-sm font-medium text-gray-900">{shift.description || "-"}</div>
                        </td>
                        <td className="px-5 py-3 whitespace-nowrap text-sm font-medium space-x-1.5">
                          <motion.button
                            whileHover={{ scale: 1.1 }}
                            whileTap={{ scale: 0.9 }}
                            onClick={() => navigate(`/workarrangement/workshift/editworkshift/${shift.id}`)}
                          ...
                          <motion.button
                            whileHover={{ scale: 1.1 }}
                            whileTap={{ scale: 0.9 }}
                            onClick={() => navigate(`/workarrangement/workshift/view/${shift.id}`)}
