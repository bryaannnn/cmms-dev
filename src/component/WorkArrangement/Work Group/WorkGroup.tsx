...
import { Plus, Upload, Filter, ChevronDown, Trash2, Edit, Eye, X, AlertTriangle, Users, Search } from "lucide-react";
import PageHeader from "../../PageHeader";
import { WorkGroup, useAuth } from "../../../routes/AuthContext";
...
const WorkGroupPage: React.FC = () => {
  ...
  const loadWorkGroup = useCallback(async () => {
    try {
      ...
      const sortedWorkGroup = [...dataWorkGroup].sort((a, b) => a.id - b.id);
  ...
  const filteredWorkGroups = useMemo(() => {
    ...
    return workGroup.filter((group) => {
      const matchesWorkGroup = group.name?.toLowerCase().includes(actualQuery);
      const matchesDescription = group.description?.toLowerCase().includes(actualQuery);

      if (category === "work group") return matchesWorkGroup;
      if (category === "description") return matchesDescription;

      return matchesWorkGroup || matchesDescription;
    });
  }, [workGroup, searchQuery, searchCategories]);

  const handleDelete = async (id: number) => {
    try {
      await deleteWorkGroup(id);
      setWorkGroup(workGroup.filter((group) => group.id !== id));
  ...
  return (
    <div className="flex h-screen font-sans antialiased bg-blue-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <PageHeader mainTitle="Work Group" mainTitleHighlight="Page" description="Manage work groups and their configurations within the system." icon={<Users />} isMobile={isMobile} toggleSidebar={toggleSidebar} />
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar bg-gray-50">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between space-y-5 md:space-y-0">
            <div>
              ...
              <p className="text-gray-600 mt-2 text-sm max-w-xl">Organize and manage work groups by specific company requirements.</p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
            <StatCard title="Total Work Groups" value={workGroup.length.toString()} change={`+${Math.floor((workGroup.length / 10) * 100)}%`} icon={<Users className="w-6 h-6" />} />
            <StatCard
              title="Active Groups"
              value={workGroup.filter((group) => group.description && group.description.length > 0).length.toString()}
              change={`+${Math.floor((workGroup.filter((group) => group.description && group.description.length > 0).length / workGroup.length) * 100)}%`}
            ...
            <div className="flex flex-col md:flex-row md:items-center md:space-x-4 space-y-4 md:space-y-0 relative">
              <div className="flex-1 relative">
                ...
                <input
                  ref={searchInputRef}
                  type="text"
                  placeholder="Search by work group or description..."
          ...
          {loading ? (
            <div className="bg-white rounded-2xl shadow-md p-8 text-center border border-blue-100">
              ...
              <p className="text-gray-600 text-base font-medium">Loading work group data...</p>
            </div>
          ) : filteredWorkGroups.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-md p-8 text-center border border-blue-100">
              <p className="text-gray-700 text-base font-medium">{searchQuery ? "No work groups found matching your search." : "No work groups available."}</p>
              {searchQuery && (
                      ...
                      >
                        ...
                        <td className="px-5 py-3 whitespace-nowrap text-sm font-medium space-x-1.5">
                          <motion.button
                            ...
                            whileTap={{ scale: 0.9 }}
                            onClick={() => navigate(`/workarrangement/workgroup/editworkgroup/${group.id}`)}
                          ...
                          <motion.button
                            whileHover={{ scale: 1.1 }}
                            whileTap={{ scale: 0.9 }}
                            onClick={() => navigate(`/workarrangement/workgroup/view/${group.id}`)}
      ...
      <Modal isOpen={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)} title="Confirm Deletion">
        <div className="space-y-5 text-center py-3">
          ...
          <p className="text-base text-gray-700 font-medium">Are you sure you want to delete this work group? This action cannot be undone.</p>
