...
const FormServiceGroup: React.FC = () => {
  const auth = useAuth();
  type AuthSubset = {
    addServiceGroup: (payload: ServiceGroupPayload) => Promise<{
      id: number | string;
      group_name: string;
      group_description?: string | null;
    }>;
  };
  const { addServiceGroup } = auth as unknown as AuthSubset;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "Failed to save service group";
      setError(message);
    } finally {
      setLoading(false);
    }
    ...
    <div className="flex h-screen font-sans antialiased bg-blue-50 text-gray-900">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-gray-100 p-4 flex items-center justify-between shadow-sm sticky top-0 z-30">
          <div className="flex items-center space-x-4">
            ...
            <h2 className="text-lg md:text-xl font-bold text-gray-900 ml-4">Add Service Group</h2>
          </div>
        </header>

        <main className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar bg-gray-50">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }} className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Create Service Group</h1>
      ...
      <Modal isOpen={showSuccessModal} onClose={handleCloseSuccess} title="Success!">
        <div className="flex flex-col items-center justify-center py-4">
          ...
          <p className="text-lg font-medium text-gray-800 text-center">Service group has been created successfully.</p>
          <motion.button
            whileHover={{ scale: 1.03 }}
            whileTap={{ scale: 0.97 }}
