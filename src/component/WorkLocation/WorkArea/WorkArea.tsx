...
import { Plus, Upload, Filter, ChevronDown, Trash2, Edit, Eye, X, AlertTriangle, Building, Search, UserIcon, Mail, Users, MapPin } from "lucide-react";
import PageHeader from "../../PageHeader";
import { WorkArea, useAuth, User } from "../../../routes/AuthContext";
...
const WorkAreaPage: React.FC = () => {
  const [workArea, setWorkArea] = useState<WorkArea[]>([]);
  const { getWorkArea, deleteWorkArea } = useAuth();
  ...
  const searchCategories = useMemo(() => [{ id: "workarea", name: "Work Area" }], []);

  const loadWorkArea = useCallback(async () => {
    try {
      setLoading(true);
      const dataWorkArea = await getWorkArea();
      const sortedWorkArea = [...dataWorkArea].sort((a, b) => a.id - b.id);
      setWorkArea(sortedWorkArea);
    } catch (err) {
      setError("Failed to load Work Area");
      console.error("Error loading Work Area:", err);
    } finally {
      setLoading(false);
    }
  }, [getWorkArea]);
  ...
  const filteredWorkArea = useMemo(() => {
    if (!searchQuery.trim()) {
      return workArea;
    }
    ...
    return workArea.filter((work) => {
      const matchesWorkArea = work.work_area.toLowerCase().includes(actualQuery);

      if (category === "work area") return matchesWorkArea;

      return matchesWorkArea;
    });
  }, [workArea, searchQuery, searchCategories]);

  const handleDelete = async (id: number) => {
    try {
      await deleteWorkArea(id);
      setWorkArea(workArea.filter((work) => work.id !== id));
      setShowDeleteConfirm(false);
      setRecordToDelete(null);
    } catch (error) {
      console.error("Failed to delete Work Area:", error);
      setError("Failed to delete Work Area. Please try again.");
    }
  };
  ...
  return (
    <div className="flex h-screen font-sans antialiased bg-blue-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <PageHeader mainTitle="Work Area" mainTitleHighlight="Page" description="Manage work area and their configurations within the system." icon={<MapPin />} isMobile={isMobile} toggleSidebar={toggleSidebar} />
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar bg-gray-50">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between space-y-5 md:space-y-0">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900">
                Work Area <span className="text-blue-600">Management</span>
              </h1>
              <p className="text-gray-600 mt-2 text-sm max-w-xl">Organize and manage work area by specific company requirements.</p>
            </div>
            <div className="flex flex-wrap gap-3 items-center">
              <motion.button
                onClick={() => navigate("/worklocation/workarea/addworkarea")}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
            <StatCard title="Total Work Area" value={workArea.length.toString()} change={`+${Math.floor((workArea.length / 10) * 100)}%`} icon={<MapPin className="w-6 h-6" />} />
          </div>
          ...
          {loading ? (
            <div className="bg-white rounded-2xl shadow-md p-8 text-center border border-blue-100">
              ...
              <p className="text-gray-600 text-base font-medium">Loading work area data...</p>
            </div>
          ) : filteredWorkArea.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-md p-8 text-center border border-blue-100">
              <p className="text-gray-700 text-base font-medium">{searchQuery ? "No work area found matching your search." : "No work area available."}</p>
              {searchQuery && (
                ...
                <table className="min-w-full divide-y divide-blue-100">
                  <thead className="bg-blue-50">
                    <tr>
                      ...
                      <th className="px-5 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Work Area</th>
                      <th className="px-5 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-blue-100">
                    {filteredWorkArea.map((work) => (
                      <motion.tr
                        key={work.id}
                        initial={{ opacity: 0, y: 5 }}
                        animate={{ opacity: 1, y: 0 }}
                      ...
                      >
                        <td className="px-5 py-3 whitespace-nowrap">
                          <div className="text-sm font-medium text-gray-900">{work.id}</div>
                        </td>
                        <td className="px-5 py-3 whitespace-nowrap">
                          <div className="text-sm font-medium text-gray-900">{work.work_area}</div>
                        </td>
                        <td className="px-5 py-3 whitespace-nowrap text-sm font-medium space-x-1.5">
                          <motion.button
                            whileHover={{ scale: 1.1 }}
                            whileTap={{ scale: 0.9 }}
                            onClick={() => navigate(`/worklocation/workarea/editworkarea/${work.id}`)}
      ...
      <Modal isOpen={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)} title="Confirm Deletion">
        <div className="space-y-5 text-center py-3">
          ...
          <p className="text-base text-gray-700 font-medium">Are you sure you want to delete this work area? This action cannot be undone.</p>
