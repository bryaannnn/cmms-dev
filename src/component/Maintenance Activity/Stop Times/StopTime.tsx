...
import { StopTimes, useAuth } from "../../../routes/AuthContext";
...
const StopTimesPage: React.FC = () => {
  ...
  const loadStopTimes = useCallback(async () => {
    try {
      ...
      const sortedStopTimes = [...dataStopTimes].sort((a, b) => a.id - b.id);
  ...
  const filteredStopTimes = useMemo(() => {
    ...
    return stoptimes.filter((stopTime) => {
      const matchesStopTime = stopTime.name?.toLowerCase().includes(actualQuery);
      const matchesDescription = stopTime.description?.toLowerCase().includes(actualQuery);

      if (category === "stop time") return matchesStopTime;
      if (category === "description") return matchesDescription;

      return matchesStopTime || matchesDescription;
    });
  }, [stoptimes, searchQuery, searchCategories]);

  const handleDelete = async (id: number) => {
    try {
      await deleteStopTimes(id);
      setStoptimes(stoptimes.filter((stopTime) => stopTime.id !== id));
      setShowDeleteConfirm(false);
      setRecordToDelete(null);
    } catch (error) {
      console.error("Failed to delete stop time:", error);
      setError("Failed to delete stop time. Please try again.");
    }
  };
  ...
  return (
    <div className="flex h-screen font-sans antialiased bg-blue-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <PageHeader mainTitle="Stop Times" mainTitleHighlight="Page" description="Manage stop times and their configurations within the system." icon={<Clock />} isMobile={isMobile} toggleSidebar={toggleSidebar} />
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar bg-gray-50">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between space-y-5 md:space-y-0">
            <div>
              ...
              <p className="text-gray-600 mt-2 text-sm max-w-xl">Organize and manage stop times by specific company requirements.</p>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:space-x-4 space-y-4 md:space-y-0 relative">
              <div className="flex-1 relative">
                ...
                <input
                  ref={searchInputRef}
                  type="text"
                  placeholder="Search by stop time or description..."
          ...
          {loading ? (
            <div className="bg-white rounded-2xl shadow-md p-8 text-center border border-blue-100">
              ...
              <p className="text-gray-600 text-base font-medium">Loading stop time data...</p>
            </div>
          ) : filteredStopTimes.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-md p-8 text-center border border-blue-100">
              <p className="text-gray-700 text-base font-medium">{searchQuery ? "No stop times found matching your search." : "No stop times available."}</p>
              {searchQuery && (
                    ...
                    {filteredStopTimes.map((stopTime) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3">
                          <div className="text-sm font-medium text-gray-900">{stopTime.description || "-"}</div>
                        </td>
                        <td className="px-5 py-3 whitespace-nowrap text-sm font-medium space-x-1.5">
                          <motion.button
                            whileHover={{ scale: 1.1 }}
                            whileTap={{ scale: 0.9 }}
                            onClick={() => navigate(`/maintenanceactivity/stoptimes/editstoptime/${stopTime.id}`)}
                          ...
                          <motion.button
                            whileHover={{ scale: 1.1 }}
                            whileTap={{ scale: 0.9 }}
                            onClick={() => handleDeleteClick(stopTime.id)}
                          ...
                          <motion.button
                            whileHover={{ scale: 1.1 }}
                            whileTap={{ scale: 0.9 }}
                            onClick={() => navigate(`/maintenanceactivity/stoptimes/view/${stopTime.id}`)}
      ...
      <Modal isOpen={showDeleteConfirm} onClose={() => setShowDeleteConfirm(false)} title="Confirm Deletion">
        <div className="space-y-5 text-center py-3">
          ...
          <p className="text-base text-gray-700 font-medium">Are you sure you want to delete this stop time? This action cannot be undone.</p>
