...
import { Plus, Upload, Filter, ChevronDown, Trash2, Edit, Eye, X, AlertTriangle, Activity, Search } from "lucide-react";
...
const ActivityPage: React.FC = () => {
  ...
  const loadActivity = useCallback(async () => {
    try {
      ...
      const sortedActivity = [...dataActivity].sort((a, b) => a.id - b.id);
  ...
  const filteredActivities = useMemo(() => {
    ...
    return activity.filter((act) => {
      const matchesActivity = act.name?.toLowerCase().includes(actualQuery);
      const matchesDescription = act.description?.toLowerCase().includes(actualQuery);

      if (category === "activity") return matchesActivity;
      if (category === "description") return matchesDescription;

      return matchesActivity || matchesDescription;
    });
  }, [activity, searchQuery, searchCategories]);
  ...
  return (
    <div className="flex h-screen font-sans antialiased bg-blue-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <PageHeader mainTitle="Activity" mainTitleHighlight="Page" description="Manage activities and their configurations within the system." icon={<Activity />} isMobile={isMobile} toggleSidebar={toggleSidebar} />
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 custom-scrollbar bg-gray-50">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between space-y-5 md:space-y-0">
            <div>
              ...
              <p className="text-gray-600 mt-2 text-sm max-w-xl">Organize and manage activities by specific company requirements.</p>
            </div>
          ...
          {loading ? (
            <div className="bg-white rounded-2xl shadow-md p-8 text-center border border-blue-100">
              ...
              <p className="text-gray-600 text-base font-medium">Loading activity data...</p>
            </div>
          ) : filteredActivities.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-md p-8 text-center border border-blue-100">
              <p className="text-gray-700 text-base font-medium">{searchQuery ? "No activities found matching your search." : "No activities available."}</p>
              {searchQuery && (
                    ...
                    {filteredActivities.map((act, index) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3">
                          <div className="text-sm font-medium text-gray-900">{act.description || "-"}</div>
                        </td>
                        <td className="px-5 py-3 whitespace-nowrap text-sm font-medium space-x-1.5">
                          <motion.button
                            whileHover={{ scale: 1.1 }}
                            whileTap={{ scale: 0.9 }}
                            onClick={() => navigate(`/maintenanceactivity/activity/editactivity/${act.id}`)}
